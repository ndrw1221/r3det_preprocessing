...
def cut_video(video, img_save_path, new_video_save_path):
    ...
    while True:
        ...
        if target_image is None:
            ...
            save = img_save_path + '/{}.jpg'.format(filename)
    ...
    if sep is True:
        ...
        for image in images[:150]:
            shutil.copy(os.path.join(oldpath, image), os.path.join(newpath_1, image))
        for image in images[150:300]:
            n = 4 - len(str(i))
            new_name = '{}.jpg'.format(i)
            new_name = str(0) * n + new_name
            shutil.copy(os.path.join(oldpath, image), os.path.join(newpath_2, new_name))
            i = i+1
